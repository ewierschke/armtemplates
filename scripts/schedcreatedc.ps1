...
$schedcreatedcDir = "${env:SystemDrive}\buildscripts\0b-schedcreatedc"
$schedcreatedcLogDir = "${schedcreatedcDir}\Logs"
$LogSource = "schedcreatedc"
$DateTime = $(get-date -format "yyyyMMdd_HHmm_ss")
$schedcreatedcLogFile = "${schedcreatedcLogDir}\schedcreatedc-log_${DateTime}.txt"
$ScriptName = $MyInvocation.mycommand.name
$ErrorActionPreference = "Stop"
$credspath = "${env:SystemDrive}\buildscripts"
$nextscript = "createdc-wwinwatch"
...
#SCRIPT
# Create the schedcreatedc log directory
New-Item -Path $schedcreatedcDir -ItemType "directory" -Force 2>&1 > $null
New-Item -Path $schedcreatedcLogDir -ItemType "directory" -Force 2>&1 > $null
...
Start-Transcript $schedcreatedcLogFile

# Create a "schedcreatedc" event log source
try {
...
do {
    try {
       Invoke-Webrequest "https://raw.githubusercontent.com/ewierschke/armtemplates/runwincustdata/scripts/${nextscript}.ps1" -Outfile "${schedcreatedcDir}\${nextscript}.ps1";
       $Stoploop = $true
       }
    catch {
        if ($Retrycount -gt 5){
...
$ADRestoreModeCredsFilePath = "${credspath}\adrestoremodepass.txt";
$ADRestoreModeKeyFilePath = "${credspath}\adrestoremodekey.txt";
$ADRestoreModeKey = Get-Content ${ADRestoreModeKeyFilePath};
$ADRestoreModePass = Get-Content ${ADRestoreModeCredsFilePath};
$ADRestoreModeSecPassword = $ADRestoreModePass | ConvertTo-SecureString -Key ${ADRestoreModeKey};
$BSTR1 = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR(${ADRestoreModeSecPassword});
$ADRestoreModeadminpass = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto(${BSTR1});
...
if ($PSVersionTable.psversion.major -ge 4) {
    $A = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-ExecutionPolicy Bypass -file ${prepcreatedcDir}\${nextscript}.ps1 -ADDomainAdminUsername '${Username}' -ADDomainAdminPw '${adminpass}' -ADRestoreModePw '${ADRestoreModeadminpass}' -ADDomainDNSName '${Domain}' -DcRole 'Replica DC'"
...
Remove-Item "${credspath}\adrestoremodepass.txt" -Force -Recurse;
Remove-Item "${credspath}\adrestoremodekey.txt" -Force -Recurse;
