...
$prepcreatedcDir = "${env:SystemDrive}\buildscripts\0a-prepcreatedc"
$prepcreatedcLogDir = "${prepcreatedcDir}\Logs"
$LogSource = "prepcreatedc"
$DateTime = $(get-date -format "yyyyMMdd_HHmm_ss")
$prepcreatedcLogFile = "${prepcreatedcLogDir}\prepcreatedc-log_${DateTime}.txt"
$ScriptName = $MyInvocation.mycommand.name
$ErrorActionPreference = "Stop"
$credspath = "${env:SystemDrive}\buildscripts"
$nextscript = "schedcreatedc"
...
#SCRIPT
# Create the prepcreatedc log directory
New-Item -Path $prepcreatedcDir -ItemType "directory" -Force 2>&1 > $null
New-Item -Path $prepcreatedcLogDir -ItemType "directory" -Force 2>&1 > $null
...
Start-Transcript $prepcreatedcLogFile

# Create a "prepcreatedc" event log source
try {
...
Invoke-Webrequest "https://raw.githubusercontent.com/ewierschke/armtemplates/runwincustdata/scripts/${nextscript}.ps1" -Outfile "${prepcreatedcDir}\${nextscript}.ps1";
...
# Write Creds to disk
log -LogTag ${ScriptName} "writing pass"
$ADRestoreModeCredsFilePath = "${credspath}\adrestoremodepass.txt"
$ADRestoreModeKeyFilePath = "${credspath}\adrestoremodekey.txt"
$ADRestoreModeSecurePwd = $ADRestoreModePw | ConvertTo-SecureString -AsPlainText -Force
$ADRestoreModeKey = New-Object Byte[] 32
[Security.Cryptography.RNGCryptoServiceProvider]::Create().GetBytes($ADRestoreModeKey)
Set-Content $ADRestoreModeKeyFilePath $ADRestoreModeKey
$ADRestoreModePass = $ADRestoreModeSecurePwd | ConvertFrom-SecureString -Key $ADRestoreModeKey
Add-Content $ADRestoreModeCredsFilePath $ADRestoreModePass
...
if ($PSVersionTable.psversion.major -ge 4) {
    $A = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-ExecutionPolicy Bypass -file ${prepcreatedcDir}\${nextscript}.ps1"
...
#Reboot controlled by domain join extension